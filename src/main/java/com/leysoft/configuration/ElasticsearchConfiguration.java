...
public class ElasticsearchConfiguration {
	...
	@Value(value = "${elasticsearch.cluster-nodes:http://localhost:9300}")
	private String clusterNode;
	
	@Value(value = "${elasticsearch.multi-threaded:true}")
	private boolean multiThreaded;
	
	@Value(value = "${elasticsearch.default-max-connection:2}")
	private int defaultMaxConnection;
	
	@Value(value = "${elasticsearch.max-connection:10}")
	private int maxConnection;
	
	@Value(value = "${elasticsearch.time-out.read}")
	private int readTimeout;
	
	@Value(value = "${elasticsearch.time-out.connection}")
